async function testUpdateWithDebug() {
  console.log('🧪 Testando atualização com debug detalhado...');
    ...
    if (beforeData.employees && beforeData.employees.length > 0) {
      ...
      if (bruno) {
        ...
        const updateResponse = await fetch(`http://localhost:3000/api/employees/${bruno.id}`, {
          ...
          body: JSON.stringify({ 
            notes: 'Teste debug - ' + new Date().toLocaleTimeString()
          })
        });
        ...
        if (updateResponse.ok) {
          ...
          console.log('   - [Employees SSE] Refetching queries');
          console.log('   - [useEmployeesQuery] Query key');
          console.log('   - [useEmployeesQuery] Fetching employees');
          ...
          if (afterData.employees && afterData.employees.length > 0) {
            ...
            if (brunoAfter) {
              ...
              console.log('✅ SSE está emitindo eventos');
              console.log('❓ React Query pode não estar invalidando corretamente');
              console.log('❓ Interface pode não estar reagindo às mudanças');
              
              console.log('\n📝 PRÓXIMOS PASSOS:');
              console.log('1. Abra o console do navegador (F12)');
              console.log('2. Vá para a página de funcionários');
              console.log('3. Atualize um funcionário');
              console.log('4. Verifique se aparecem os logs de debug');
              console.log('5. Se não aparecerem, o problema está na invalidação');
