    ...
    if (beforeData.employees && beforeData.employees.length > 0) {
      ...
      if (bruno) {
        ...
        const updateResponse = await fetch(`http://localhost:3000/api/employees/${bruno.id}`, {
          ...
          body: JSON.stringify({ 
            notes: 'Teste interface - ' + new Date().toLocaleTimeString()
          })
        });
        ...
        if (updateResponse.ok) {
          ...
          if (afterData.employees && afterData.employees.length > 0) {
            ...
            if (brunoAfter) {
              ...
              console.log('✅ companyFunctionId está sendo usado');
              console.log('✅ companyFunction.name está sendo retornado');
              console.log('✅ Lógica da tabela está correta');
              console.log('✅ Cargo está sendo calculado corretamente');
              ...
              console.log('5. Se não atualizar, o problema está no React Query ou SSE');
