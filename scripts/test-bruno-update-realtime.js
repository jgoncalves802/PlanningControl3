async function testBrunoUpdateRealtime() {
  console.log('🧪 Testando atualização em tempo real do BRUNO...');
    ...
    if (beforeData.employees && beforeData.employees.length > 0) {
      ...
      if (bruno) {
        ...
        const updateResponse = await fetch(`http://localhost:3000/api/employees/${bruno.id}`, {
          ...
          body: JSON.stringify({ 
            notes: 'Teste atualização em tempo real - ' + new Date().toLocaleTimeString()
          })
        });
